...
puts "Sent SETTINGS with INITIAL_WINDOW_SIZE=3"
...
puts "Received: #{settings_ack1.class} ACK=#{settings_ack1.as(HT2::SettingsFrame).flags.ack?}"
...
puts "Sent HEADERS frame (stream 1)"
...
2.times do
  ...
  when HT2::HeadersFrame
    puts "Received HEADERS frame"
    headers_received = true
  when HT2::DataFrame
    puts "Received DATA frame: length=#{frame.data.size}"
...
puts "Total data received so far: #{data_received} bytes"
...
puts "Sent SETTINGS with INITIAL_WINDOW_SIZE=2 (window should be negative: 2 - #{data_received} = #{2 - data_received})"

# Read SETTINGS ACK
settings_ack2 = HT2::Frame.read_from(client)
puts "Received: #{settings_ack2.class} ACK=#{settings_ack2.as(HT2::SettingsFrame).flags.ack?}"
...
puts "Sent WINDOW_UPDATE with increment=2 for stream 1"
  ...
  begin
    ...
    when HT2::DataFrame
      if frame.stream_id == 1
        puts "Received DATA frame: length=#{frame.data.size}, flags=#{frame.flags.value}"
        if frame.data.size == 1 && !frame.flags.end_stream?
          puts "✓ Test PASSED: Server correctly tracked negative window and sent 1 byte after update"
        else
          puts "✗ Test FAILED: Expected DATA(length:1, flags:0x00), got DATA(length:#{frame.data.size}, flags:0x#{frame.flags.value.to_s(16).rjust(2, '0')})"
        end
        break
      end
    when HT2::GoAwayFrame
      puts "Received GOAWAY: last_stream_id=#{frame.last_stream_id}, error_code=#{frame.error_code}"
      break
    else
      puts "Received unexpected frame: #{frame.class}"
    end
  rescue IO::TimeoutError
    puts "Timeout waiting for frames"
    break
  rescue ex
    puts "Error reading frame: #{ex.message}"
    break
  end
end
