...
puts "Sent SETTINGS with INITIAL_WINDOW_SIZE=0"
...
puts "Sent SETTINGS with INITIAL_WINDOW_SIZE=1"
...
while Time.monotonic - start_time < timeout
  begin
    ...
    case frame
    when HT2::DataFrame
      puts "Received DATA frame: stream_id=#{frame.stream_id}, length=#{frame.data.size}, flags=#{frame.flags.value}"
      if frame.stream_id == 1
        if frame.data.size == 1 && !frame.flags.end_stream?
          puts "✓ Test PASSED: Server correctly adjusted stream window and sent 1 byte"
        else
    ...
    when HT2::HeadersFrame
      if frame.stream_id == 1
        puts "Received HEADERS frame for stream 1"
        # Continue to wait for DATA
      end
    when HT2::WindowUpdateFrame
      puts "Received WINDOW_UPDATE: stream_id=#{frame.stream_id}, increment=#{frame.window_size_increment}"
