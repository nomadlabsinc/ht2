...
describe HT2::Security do
  describe "CVE-2019-9513: Resource Loop" do
    it "validates frame size against MAX_FRAME_SIZE setting" do
      # Create a mock socket that provides oversized frame
      mock_socket = IO::Memory.new
      
      # Write client preface
      mock_socket.write(HT2::CONNECTION_PREFACE.to_slice)
      
      # Write SETTINGS frame from client (required after preface)
      settings_frame = HT2::SettingsFrame.new
      mock_socket.write(settings_frame.to_bytes)
      
      # Create oversized frame header
      oversized_frame = Bytes.new(HT2::Frame::HEADER_SIZE)
      # Length: 20000 (exceeds default MAX_FRAME_SIZE of 16384)
      oversized_frame[0] = 0x00
      oversized_frame[1] = 0x4E
      oversized_frame[2] = 0x20
      # Type: DATA
      oversized_frame[3] = 0x00
      # Flags: None
      oversized_frame[4] = 0x00
      # Stream ID: 1
      ...
      server = HT2::Connection.new(mock_socket, is_server: true)
      
      # The frame size validation happens in read_loop
      error_raised = false
      spawn do
        begin
          server.start
        rescue ex : HT2::ConnectionError
          error_raised = true if ex.message.to_s.matches?(/Frame size \d+ exceeds maximum/)
        end
      end
